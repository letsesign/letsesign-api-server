...
class Caller {
  constructor() {
      ...
      try {
        ...
        if (isPreview) {
          try {
            ...
            const preViewPdf = await renderer.renderPDF(internalTaskConfig, internalTemplateInfo, pdfFileDataBuffer);
    ...
    this.submitTaskWithTemplate = async (
      apiKey,
      bearerSecret,
      kmsPublicKey,
      taskConfig,
      ...
      try {
        ...
        if (isPreview) {
          try {
            const renderer = new Renderer();
            const preViewPdf = await renderer.renderPDF(
              internalTaskConfig,
              internalTemplateInfo,
              parsedTemplate.pdfFileDataBuffer
            );
    ...
    this.submitBulkTaskWithTemplate = async (
      apiKey,
      bearerSecret,
      kmsPublicKey,
      taskConfig,
